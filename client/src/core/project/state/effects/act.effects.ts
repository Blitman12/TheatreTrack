...
export class ActEffects extends BaseEffects {
    requestAddAct$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap((action) => this._projectService.addAct(action.projectId, action.name).pipe(
            map(res => this.handleResponse(res, actActions.addActSuccess(), 'Something went wrong')),
            catchError((error) => this.handleFailure(error, actActions.addActFailure()))
        ))
    ));
    ...
    requestEditAct$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap((action) => this._projectService.editAct(action.id, action.name).pipe(
            map(res => this.handleResponse(res, actActions.editActSuccess(), 'Something went wrong')),
            catchError((error) => this.handleFailure(error, actActions.editActFailure()))
        ))
    ));
    ...
    requestDeleteAct$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap((action) => this._projectService.deleteAct(action.id).pipe(
            map(res => this.handleResponse(res, actActions.deleteActSuccess(), 'Something went wrong')),
            catchError((error) => this.handleFailure(error,actActions.deleteActFailure()))
        ))
    ));
