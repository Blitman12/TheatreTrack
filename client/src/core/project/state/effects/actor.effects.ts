...
export class ActorEffects extends BaseEffects {
    requestActors$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap(() => this._projectService.getActors().pipe(
            map(actors => this.handleResponse(actors,  actorActions.loadActorsSuccess({ actors }), 'Something went wrong')),
            catchError((error) => this.handleFailure(error, actorActions.loadActorsFailure()))
        ))
    ));
    ...
    requestAddActor$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap((action) => this._projectService.addActor(action.firstName, action.lastName, action.age, action?.currentCharacter).pipe(
            map(res => this.handleResponse(res, actorActions.addActorSuccess(), 'Something went wrong')),
            catchError((error) => this.handleFailure(error, actorActions.addActorFailure()))
        ))
    ));
    ...
    requestEditActor$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap((action) => this._projectService.editActor(action.id, action?.firstName, action?.lastName, action?.age, action?.currentCharacter).pipe(
            map(res => this.handleResponse(res,  actorActions.editActorSuccess(), 'Something went wrong')),
            catchError((error) => this.handleFailure(error, actorActions.editActorFailure()))
        ))
    ));
    ...
    requestDeleteActor$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap((action) => this._projectService.deleteActor(action.id).pipe(
            map(res => this.handleResponse(res, actorActions.deleteActorSuccess(), 'Something went wrong')),
            catchError((error) => this.handleFailure(error,actorActions.deleteActorFailure()))
        ))
    ));
