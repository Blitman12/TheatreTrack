...
export class ActorSideBarComponent extends BaseComponent implements OnInit {
  ...
  public delete(actor: Actor): void {
    const confirmDelete = window.confirm(
      'Delete Actor? There is not going back'
    );
    if (confirmDelete) {
  ...
  private setupSubscriptions(): void {
    this._subscriptions.push(
      this._projectSelectors.actorInfo$.subscribe((actors) =>
        this.handleInitialActors(actors)
      ),
      this._projectSelectors.actorToggleState.subscribe(
        (toggleState) => (this.opened = toggleState)
      )
    );
  }
